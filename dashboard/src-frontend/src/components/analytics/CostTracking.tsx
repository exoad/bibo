...
export function CostTracking() {
	...
	const costData = useMemo(() => {
		...
		sessions.forEach((session) => {
			...
			modelCosts[normalizedModel] =
				(modelCosts[normalizedModel] || 0) + sessionCost;
		...
		const recentCost = dates
			.slice(-7)
			.reduce((sum, date) => sum + dailyCosts[date], 0);
		const previousCost = dates
			.slice(-14, -7)
			.reduce((sum, date) => sum + dailyCosts[date], 0);
		const trend =
			...
			<div className="space-y-3">
				...
				<div className="flex items-center justify-between">
					...
					<div className="flex items-center gap-2">
						...
						{costData.trend !== 0 && (
							<div
								className={`flex items-center gap-0.5 text-xs ${costData.trend > 0 ? "text-red-bright" : "text-green-bright"}`}
							>
								{costData.trend > 0 ? (
							...
							.map(([model, usage]) => (
								<div key={model} className="flex items-center justify-between">
									<div className="flex flex-col min-w-0 flex-1 mr-2">
										<span className="text-xs text-fg4 truncate" title={model}>
											{model}
										</span>
										<span className="text-xs text-fg-muted">
											{usage.sessions} sess · {usage.tokens.toLocaleString()}{" "}
											tok
										</span>
									</div>
									<span className="text-xs font-medium text-fg1">
