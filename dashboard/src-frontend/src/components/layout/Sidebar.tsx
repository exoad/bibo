...
export function Sidebar() {
	...
	return (
		<>
			...
			>
				{isOpen ? (
					<X className="w-5 h-5 text-fg1" />
				) : (
					<List className="w-5 h-5 text-fg1" />
				)}
			</button>
			...
			{/* Sidebar panel */}
			<nav
				className={`fixed lg:static inset-y-0 left-0 z-40 lg:z-auto w-[260px] lg:w-[220px] bg-bg0-hard border-r border-bg2 flex flex-col transform lg:transform-none transition-transform duration-300 ease-in-out ${
								...
								<NavLink
									...
									onClick={() => {
										useConfigStore
											.getState()
											.setActiveView(
												item.path.replace("/", "") as typeof activeView,
											);
										setIsOpen(false);
									}}
