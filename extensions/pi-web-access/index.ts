...
type WebSearchWorkflow = "none" | "summary-review" | "auto-approve";
type CuratorWorkflow = "summary-review" | "auto-approve";
...
function resolveWorkflow(input: unknown, hasUI: boolean): WebSearchWorkflow {
	...
	if (typeof input === "string") {
		...
		if (normalized === "auto-approve") return "auto-approve";
	}
...
export default function (pi: ExtensionAPI) {
	...
	async function loadSummaryModelChoices(
		...
		if (
			configuredSummaryModel.length > 0 &&
			availableValues.has(configuredSummaryModel)
		) {
	...
	async function handleAutoApprove(
		...
		pc.onUpdate?.({
			content: [
				{
					type: "text",
					text: "Auto-approve mode: generating summary without browser approval...",
				},
			],
			details: { phase: "auto-approving", progress: 0.8 },
		});

		// Wait a tick to ensure all search results are collected
		await new Promise((resolve) => setTimeout(resolve, 100));

		// Generate summary using all search results
		const allIndices = [...pc.searchResults.keys()];
		const { summary, meta } = await generateSummaryForSelectedIndices(
			allIndices,
			pc.searchResults,
			pc.summaryContext,
			pc.signal ?? new AbortController().signal,
			pc.defaultSummaryModel ?? undefined,
		);

		pc.onUpdate?.({
			content: [{ type: "text", text: "Summary generated (auto-approved)" }],
			details: { phase: "completed", progress: 1 },
		});

		// Build return with all results and the generated summary
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			workflow: Type.Optional(
				StringEnum(["none", "summary-review", "auto-approve"], {
					description:
						"Search workflow mode: none = no curator, summary-review = open curator with auto summary draft (default), auto-approve = generate summary without waiting for user approval",
				}),
			),
		}),
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			if (shouldCurate) {
				...
				const curatorWorkflow: CuratorWorkflow =
					workflow === "auto-approve" ? "auto-approve" : "summary-review";
				...
				// For auto-approve workflow, skip browser and auto-generate summary
				if (curatorWorkflow === "auto-approve") {
					pc.browserPromise = handleAutoApprove(pc, finish);
				} else {
	...
	pi.registerCommand("curator", {
		...
		handler: async (args, ctx) => {
			...
			if (arg.length === 0) {
				...
				// Cycle through: none -> summary-review -> auto-approve -> none
				if (current === "none") {
					newWorkflow = "summary-review";
				} else if (current === "summary-review") {
					newWorkflow = "auto-approve";
				} else {
					newWorkflow = "none";
				}
			} else if (arg === "on") {
				...
				ctx.ui.notify(
					`Unknown option: ${arg}. Use on, off, none, summary-review, or auto-approve.`,
					"error",
				);
				return;
			}
			...
			let label: string;
			if (newWorkflow === "none") {
				label = "Curator disabled — web_search will return raw results";
			} else if (newWorkflow === "auto-approve") {
				label =
					"Auto-approve enabled — web_search will generate summary without browser approval";
			} else {
				label =
