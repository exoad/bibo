...
function tryExec(command: string, timeout = 5000): string | null {
	try {
		return execSync(command, { encoding: "utf-8", timeout, cwd: BIBO_ROOT }).trim();
	} catch {
		return null;
	}
}
...
async function checkNodeVersion(): Promise<CheckResult> {
	...
	return { name: "Node.js Version", status: "warn", message: version, details: "Node.js 20+ recommended" };
}
...
async function checkBiboStructure(): Promise<CheckResult> {
	...
	if (missing.length === 0) {
		return { name: "Bibo Structure", status: "pass", message: "All required files present" };
	}
	return { name: "Bibo Structure", status: "fail", message: `Missing: ${missing.join(", ")}` };
}

async function checkProviderConfig(): Promise<CheckResult> {
	const providerPath = join(BIBO_ROOT, "provider.json");
	if (!existsSync(providerPath)) {
		return { name: "Provider Config", status: "fail", message: "provider.json not found", details: "Copy provider.json.example to provider.json" };
	}
	const config = tryReadJson(providerPath);
	if (!config || typeof config !== "object") {
		return { name: "Provider Config", status: "fail", message: "Invalid JSON in provider.json" };
	}
	const providers = (config as Record<string, unknown>).providers;
	if (!Array.isArray(providers) || providers.length === 0) {
		return { name: "Provider Config", status: "warn", message: "No providers configured" };
	}
	return { name: "Provider Config", status: "pass", message: `${providers.length} provider(s) configured` };
}
...
async function checkProviderConnectivity(): Promise<CheckResult> {
	...
	if (!config?.providers) {
		return { name: "Provider Connectivity", status: "warn", message: "Skipped (no config)" };
	}

	const providers = config.providers as Array<{ baseUrl?: string; name?: string }>;
	...
	for (const provider of providers) {
		...
		try {
			...
			execSync(`curl -s -o /dev/null -w "%{http_code}" --max-time 3 "${url}/models" 2>/dev/null || echo "000"`, { encoding: "utf-8", timeout: 5000 });
	...
	if (results.length === 0) {
		return { name: "Provider Connectivity", status: "warn", message: "No providers to check" };
	}
...
async function checkSettings(): Promise<CheckResult> {
	...
	if (!existsSync(settingsPath)) {
		return { name: "Settings", status: "fail", message: "settings.json not found" };
	}
...
async function checkExtensions(): Promise<CheckResult> {
	if (!existsSync(EXTENSIONS_DIR)) {
		return { name: "Extensions", status: "fail", message: "Extensions directory not found" };
	}
	try {
		const entries = readdirSync(EXTENSIONS_DIR, { withFileTypes: true });
		const extensions = entries.filter((e) => e.isDirectory()).map((e) => e.name);
		return { name: "Extensions", status: "pass", message: `${extensions.length} extensions found` };
	} catch (e) {
...
async function checkNpmPackages(): Promise<CheckResult> {
	...
	if (!existsSync(packagePath)) {
		return { name: "NPM Packages", status: "fail", message: "package.json not found" };
	}

	const nodeModulesPath = join(BIBO_ROOT, "node_modules");
	if (!existsSync(nodeModulesPath)) {
		return { name: "NPM Packages", status: "fail", message: "node_modules not found", details: "Run: npm install" };
	}

	// Check for key packages
	const keyPackages = ["@mariozechner/pi-coding-agent", "@sinclair/typebox"];
	const missing = keyPackages.filter((pkg) => !existsSync(join(nodeModulesPath, pkg)));

	if (missing.length > 0) {
		return { name: "NPM Packages", status: "warn", message: `Missing: ${missing.join(", ")}` };
	}

	return { name: "NPM Packages", status: "pass", message: "Core dependencies installed" };
}

async function checkCostTrackerState(): Promise<CheckResult> {
	const statePath = join(homedir(), ".pi", "extensions", "cost-tracker", "state.json");
	if (!existsSync(statePath)) {
		return { name: "Cost Tracker", status: "warn", message: "No state file (will be created on first run)" };
	}
	try {
		const state = tryReadJson(statePath) as { totalCost?: number } | null;
		const cost = state?.totalCost ?? 0;
		return { name: "Cost Tracker", status: "pass", message: `Accumulated: $${(cost / 100).toFixed(2)}` };
	} catch {
		return { name: "Cost Tracker", status: "warn", message: "Corrupt state file" };
	}
}
...
async function checkBrainVault(): Promise<CheckResult> {
	...
	if (hasVault) {
		try {
			...
			const noteCount = readdirSync(vaultPath).filter((f) => f.endsWith(".md")).length;
...
async function checkCheckpoints(): Promise<CheckResult> {
	...
	if (!existsSync(checkpointsPath)) {
		return { name: "Checkpoints", status: "warn", message: "No checkpoints yet" };
	}
	try {
		const sessions = readdirSync(checkpointsPath).filter((f) =>
			statSync(join(checkpointsPath, f)).isDirectory()
		);
		return { name: "Checkpoints", status: "pass", message: `${sessions.length} session(s) backed up` };
	} catch {
		return { name: "Checkpoints", status: "warn", message: "Error reading checkpoints" };
	}
}
...
async function checkGitStatus(): Promise<CheckResult> {
	...
	return { name: "Git", status: "warn", message: `${lines.length} uncommitted change(s)` };
}
...
async function checkEnvironment(): Promise<CheckResult> {
	...
	if (setVars.length === 0) {
		return { name: "Environment", status: "pass", message: "No overrides (using defaults)" };
	}
	return { name: "Environment", status: "pass", message: `${setVars.length} override(s) active` };
}

// ── Main Doctor Function ─────────────────────────────────────────────

async function runDiagnostics(): Promise<DiagnosticReport> {
	const packageJson = tryReadJson(join(BIBO_ROOT, "package.json")) as { version?: string; dependencies?: Record<string, string> } | null;
	const piVersion = packageJson?.dependencies?.["@mariozechner/pi-coding-agent"] ?? "unknown";
...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		name: "doctor",
		label: "Doctor",
		description: "Run comprehensive diagnostics on bibo setup. Trigger: /doctor",
