...
web_search({ queries: ["query 1", "query 2"], workflow: "auto-approve" });
```
...
| `workflow`          | `none` (skip curator), `summary-review` (open curator with summary draft, default), or `auto-approve` (generate summary without browser approval) |
...
```
/curator                    # cycle through: none -> summary-review -> auto-approve -> none
/curator on                 # enable curator (summary-review)
/curator off                # disable curator (raw results only)
/curator summary-review     # explicit workflow (browser approval)
/curator auto-approve       # auto-generate summary without browser approval
```

Persists to `~/.pi/web-search.json` and takes effect on the next `web_search` call.

- `none`: Returns raw results without opening the curator
- `summary-review`: Opens browser for manual approval of generated summary
- `auto-approve`: Generates summary automatically without waiting for browser approval
