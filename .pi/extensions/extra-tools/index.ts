...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			path: Type.Optional(
				Type.String({ description: "Base directory (default: cwd)" }),
			),
		}),
		async execute(_id, { pattern, path }) {
			try {
				...
				matches.sort();
				const text =
					matches.length === 0 ? "No files matched" : matches.join("\n");
				return {
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			prompt: Type.Optional(
				Type.String({
					description: "Hint for what to extract (informational)",
				}),
			),
		}),
	...
	pi.registerTool({
		...
		async execute(_id, { query }) {
			try {
				...
				const titleRe =
					/class="result__title"[^>]*>[\s\S]*?<a[^>]*href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/g;
