...
export default function (pi: ExtensionAPI) {
	...
	pi.on("before_agent_start", async (event, ctx) => {
		...
		// Cap knowledge budget to avoid context bloat, but allow more for fast models
		budget = Math.min(budget, 250);
		...
		let used = 0;
		// Allow up to 3 knowledge entries for fast models that can handle more context
		const maxEntries = 3;
